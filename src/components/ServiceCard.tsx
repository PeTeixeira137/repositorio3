...
export function ServiceCard({ ordem }: ServiceCardProps) {
    return (
        <div className="bg-white shadow-md rounded-lg p-4 mb-4">
            ...
            <span className={`inline-block px-2 py-1 text-sm font-semibold rounded-full ${ordem.status === 'aberto' ? 'bg-red-500 text-white' : ordem.status === 'em andamento' ? 'bg-yellow-500 text-white' : 'bg-green-500 text-white'}`}>
                {ordem.status}
            </span>
        </div>
    )
